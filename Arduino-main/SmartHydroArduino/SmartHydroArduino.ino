...
#include <WiFiEspServer.h> //sets up communication line to a user 
#include <DHT.h> //sensor for temp and humidity
#include "DFRobot_PH.h"  //measures and calibrtes pH 
#include "DFRobot_EC10.h" //measures and calibrtes EC 
...
//random forest models are implemented to predict th eoptimal conditions for ec, ph, humidity and temp which will change based on water pumps and fans
...
RingBuffer buf(16); //to process incoming data for web api
...
const unsigned long SIXTEEN_HR = 57600000; //sixteen hour interval to check ec and pH
...
float lightLevel;
float flowRate;
volatile int pulseCount = 0; //water flow sensor
...
void setup() {
  ...
  pinMode(FLOW_PIN, INPUT);
   ...
   sei();
    ...
    for (int i = 3; i < 13; i++)
     {
      if (i != 8) {
        pinMode(i, OUTPUT); //initialises all components as output modules
        togglePin(i);
      }
     }
  ...
  //schedules automatic readings of sensors
    ...
    while (client.connected()) {  // Loop while the client is connected
      if (client.available()) {   // Check if data is available from the client
        ...
        if (buf.endsWith("\r\n\r\n")) {
          ...
          //sensor readings are sent to web interface as a json file format
          break;
        }
        ...
        if (buf.endsWith("/components")) {
          ...
          + String(digitalRead(EXTRACTOR_PIN)) + "\",\n  \"Fan\": \"" + String(digitalRead(FAN_PIN)) +  "\"\n }"; 
        }
      }
    }
...
//methods that extract and return the sensor readings
float getLightLevel() {
...
void setComponent(int result, int pin, int status){
    if (result == 0) { // if component is Below Optimal
    if (status == 1){ //if component is on
    digitalWrite(pin, LOW); //switch off
    //Serial.println("FAN offfffff");
    } 
  }
  else if (result == 1) { //if component is Above Optimal
    if (status == 0){ //if the component is off
      digitalWrite(pin, HIGH); //switch on
      //Serial.println("FAN ON");
    } 
  }
  else {
     if (status == 0){ //Optimal if the component is off
...
//machine learning models implemented to predict optimal conditions of components
...
//methods stop the adjustment processes of the sensors
void disablePH() {
...
//returns the calculated instantaneous flow rate in liters per hour
float getFlowRate() {
  currentTime = millis();

  if (currentTime >= (cloopTime + 1000)) { //check that time has passed since last reading measurement
    cloopTime = currentTime;
    float flowRatePerHr = (pulseCount * 60 / 7.5); //7.5 is sensor specific (calibration)
    pulseCount = 0;
    return flowRatePerHr;
  }
}

//methods to control grow light cycle
void toggleLightOn() {
